...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    //处理cgi
    if(cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3') /* *(p+1) == '2'表示 登录，'3'表示注册*/)
    {
        ...
        else if(1 == m_SQLVerify)
        {
            ...
            else if (*(p + 1) == '2')
            {
                ...
                if(pid == 0) /* 子进程 */
                {
                    ...
                    //关闭管道的读端
                    close(pipefd[0]);
                    //执行cgi程序，m_real_file,name,password为输入
...
bool http_conn::write()
{
    ...
    if(bytes_to_send == 0)
    {
        utils_.modfd(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);
        init();
        return true;
    }
    ...
    while(1)
    {
        ...
        if(temp < 0)
        {
            if(errno == EAGAIN)
            { 
                utils_.modfd(m_epollfd, m_sockfd, EPOLLOUT, m_TRIGMode);
                return true;
            }
            unmap();
            return false;
        ...
        /* 发送大文件且非第一次发送，要重置iv[0]的长度，更新iv[1]的初始位置 */
        if(bytes_have_send >= m_iv[0].iov_len /* 需要两块内存 */)
        {
            m_iv[0].iov_len = 0;
...
void http_conn::process()
{
    ...
    if(!write_ret) /* 写失败就关闭连接 */
    {
        close_conn();
    }
    utiles_.modfd(m_epollfd, m_sockfd, EPOLLOUT, m_TRIGMode);
}
